### 工作目录约定（必须）
所有下载命令都增加参数：`--work-dir <work-dir>`，用于固定输出目录。
本技能固定下载根目录：`./downloads`。
禁止下载到 `scripts/` 目录。
建议每次任务使用独立子目录：`./downloads/<BV>_p<页码>`。

### 字幕下载示例
bbdown BV1ztcjzwEiN --sub-only --work-dir "./downloads/BV1ztcjzwEiN_p1"

### 存在分P的视频字幕下载示例
bbdown BV1CAxaeHEeH --sub-only -p 2 --work-dir "./downloads/BV1CAxaeHEeH_p2"

### 没有原生字幕只有AI字幕下载示例
bbdown BV1CAxaeHEeH --sub-only -p 2 --skip-ai false --work-dir "./downloads/BV1CAxaeHEeH_p2"

### 推荐两段式下载策略（解决无字幕输出）
1. 先尝试：`--sub-only`
2. 若未产出字幕文件，立即重试：`--sub-only --skip-ai false`

### 字幕文件定位策略（解决文件名不稳定）
在 `--work-dir` 下递归查找 `*.srt/*.ass/*.json/*.vtt`，命中多个时选择最近修改文件；不要手写目标文件名。
...
Options:
  ...
  -p, --select-page <select-page>                选择指定分p或分p范围: (-p 8 或 -p 1,2 或 -p 3-5 或 -p ALL 或 -p LAST 或 -p
                                                 3,5,LATEST)
  ...
  --work-dir <work-dir>                          设置程序的工作目录（建议始终显式传入）
